...
cat ${script_folder}/raw/data.json | \
    jq -r '.[].Matches | .[].Matches | .[].Line' | \
    sed -e 's,[ ]*as .*,,' -e 's,[ ]*# .*,,' -e 's,from ,,g' -e 's, import ,.,g' -e 's,^import ,,g' -e 's,tempest.lib,tempest#lib,g'| \
    cut -d'.' -f1,2 | sed -e 's,tempest#lib,tempest.lib,g' > ${script_folder}/raw/all_lines
...
for filter in 'tempest' 'lib'; do
    ...
    echo "import;namespace;count;total_count;repos" > ${script_folder}/repo_per_${filter}_imports
    for import in $(cat ${script_folder}/imports | egrep ${grep_opt} '^tempest.lib'); do
        printf "${import};${import#tempest.*};"
        printf "$(egrep -c ${import}'(,|$)' ${script_folder}/imports_per_repo);"
        printf "$(egrep -c ${import} ${script_folder}/raw/all_lines);"
        printf "$(egrep ${import}'(,|$)' ${script_folder}/imports_per_repo | cut -d';' -f1 | tr '\n' ',' | sed -e 's/,$//')"
        printf "\n"
